    ...
    defineAdditionalTests({ suite, it }) {
        ...
        suite('Offline timestamp persistence', getHarness => {
            ...
            it('does not reset offlineSince when a server is still offline after restart', async () => {
                ...
                await setState(harness, `${prefix}.servers.server1.offlineSince`, {
                    val: originalOfflineSince,
                    ack: true,
                });
        ...
        suite('Clean shutdown', getHarness => {
            ...
            before(async () => {
                ...
                mockServer = http.createServer((_request, _response) => {
                    markStarted();
                    // Intentionally keep the response open. The adapter must still be able
                    // to unload promptly while a fetch request is pending.
                });
                baseUrl = await listen(mockServer);
            });
            ...
            it('stops cleanly while an HTTP request is active', async function () {
                this.timeout(15_000);
                const harness = getHarness();
                ...
                await Promise.race([
                    requestStarted,
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Mock request did not start')), 5000)),
                ]);

                await Promise.race([
                    harness.stopAdapter(),
                    new Promise((_, reject) =>
                        setTimeout(() => reject(new Error('Adapter did not stop while request was active')), 5000),
                    ),
                ]);
